...
/** B-10 베스트 */
export default async function BestPage({ searchParams }: { searchParams?: { lang?: string } }) {
      ...
      <ul className="space-y-3">
        {data.best.map((b) => (
          <li key={b.slug} className="rounded-xl border border-[var(--theme-border)] bg-[var(--theme-surface)] p-4">
            ...
            <Link className="mt-2 block text-lg font-medium text-[var(--theme-accent)] hover:text-[var(--theme-accent-strong)] hover:underline" href={withLang(`/blog/${b.slug}`)}>
              {b.title}
            </Link>
